...
namespace RakNet
{
	...
	public:
		/// Default Constructor
		BitStream();
		void FilePrint(const char* outputType, const char* outputString)
		{
			if (outputString != NULL)
			{
			
			FILE* f = fopen("packet.txt", "a");
			fputs(outputType, f);
			fputs(" :", f);
			fputs(outputString, f);
			fputs("\n", f);
...
#pragma warning(disable:4127)   // conditional expression is constant
	...
	template <>
		...
		{
			cout << " bool :" << inTemplateVar;
			char b[4] = { 0, };
			itoa(inTemplateVar, b, 10);

			FilePrint("bool", (const char*)inTemplateVar);
	...
	{
			cout << " SystemAddress :" << inTemplateVar.binaryAddress <<" port : "<<inTemplateVar.port;
			char b[20] = { 0, };
			itoa(inTemplateVar.binaryAddress, b, 10);
			FilePrint("SystemAddress", (const char*)inTemplateVar.binaryAddress);
			memset(b, 0, sizeof(b));
			itoa(inTemplateVar.port, b, 10);
			FilePrint("port", (const char*)b);
	...
	{
		cout << " uint24_t : " << inTemplateVar;
		char b[20] = { 0, };
		itoa(inTemplateVar, b, 10);
		FilePrint("uint24_t", (const char*)b);
	...
	template <>
		...
		{
			cout << " RakNetGUID :" << inTemplateVar.g;
			char b[20] = { 0, };
			itoa(inTemplateVar.g, b, 10);
			FilePrint("RakNetGUID", (const char*)b);
...
//		if (NetworkID::IsPeerToPeerMode()) // Use the function rather than directly access the member or DLL users will get an undefined external error
		...
		*/
		cout << " NetWorkID :" << inTemplateVar.localSystemAddress;
		char b[30] = { 0, };

		itoa(inTemplateVar.localSystemAddress, b, 10);
		FilePrint("NetWorkID", (const char*)b);
	...
	{
		FilePrint("String",(const char*)inTemplateVar);
